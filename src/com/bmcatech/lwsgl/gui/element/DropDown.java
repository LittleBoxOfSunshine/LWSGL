...
import com.bmcatech.lwsgl.gui.SmartImage;

import java.awt.*;

public class DropDown extends com.bmcatech.lwsgl.gui.element.Component {
		public com.bmcatech.lwsgl.gui.element.Button[] options;
		...
		public DropDown(String imageLocation, String hoverLocation, String l, int xpos, int ypos, String optionLocation, String optionHoverLocation, String[] options){
			...
			norm = i.clone();
			this.i = i.clone();
			this.options = new com.bmcatech.lwsgl.gui.element.Button[options.length];
			isOpen = false;
			...
			for(int z=0; z<options.length; z++)
				this.options[z] = new com.bmcatech.lwsgl.gui.element.Button(optionLocation, optionHoverLocation, options[z], xpos+width-((new SmartImage(optionLocation)).getWidth()), positions[z]);
		}
		...
		public DropDown(String imageLocation, String l, int xpos, int ypos, String optionLocation, String[] options){
			...
			int[] positions = centerOptions(ypos, options.length, new SmartImage(optionLocation).getHeight(), height);
			for(int z=0; z<options.length; z++)
				this.options[z] = new com.bmcatech.lwsgl.gui.element.Button(optionLocation, options[z], xpos+width-((new SmartImage(optionLocation)).getWidth()), positions[z]);
		}
